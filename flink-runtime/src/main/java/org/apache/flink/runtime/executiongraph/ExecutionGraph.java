...
/**
 * todo
 * 	 executionGraph 是在JobManager中生成的, Client向JobManager 提交JobGraph后,
 * 	 jobManager 就会根据JobGraph来创建对应的ExecutionGraph, 并以此来调度任务
 *
 *
 * todo
 * 	 ExecutionGraph是在创建JobMaster时就构建完成的, 之后就可以被调度执行。
 */
public class ExecutionGraph implements AccessExecutionGraph {
	...
	public void attachJobGraph(List<JobVertex> topologiallySorted) throws JobException {
		...
		/**
		 * todo
		 * 	  按照拓扑排序的结果依次为每个JobVertex 创建对应的ExectionJobVertex
		 */
		for (JobVertex jobVertex : topologiallySorted) {
			 ...
			 * todo
			 * 		创建ExectionJobVertex
			 * 		在创建ExectionJobVertex的时候会创建对应的
			 * 			ExectionVertex, IntermediateResult, ExecutionEdge, IntermediateResultPartition等对象
			 *
			 * todo
			 * 		>> 每一个JobVertex对应一个ExecutionJobVertex
			 * 		>> 每一个ExecutionJobVertex有parallelism个ExecutionVertex
			 * 		>> 每个JobVertex可能有(n>=0)个IntermediateDataSet
			 * 			在ExecutionJobVertex中一个IntermediateDataSet对应一个IntermediateResult
			 * 			每个IntermediateResult都有parallelism个生产者,对应parallelism个IntermediateResultPartition
			 * 		>> 每一个ExecutionJobVertex都会和前向的IntermediateResult连接,
			 * 				实际上是ExecutionVertex和IntermediateResult建立连接, 生产ExectionEdge
			 */
			ExecutionJobVertex ejv = new ExecutionJobVertex(
					...
					globalModVersion,
					createTimestamp);
			// todo 将创建的ExecutionJobVertex连接上游的IntermediateResult
			...
			// todo 节点总数需要加上当前执行图节点的并行度, 因为执行图是作业图的并行化
	...
	/**
	 * todo
	 * 	按照拓扑顺序为所有的ExecutionJobVertex分配资源, 其中每一个ExecutionVertex都需要分配一个slot
	 * 	ExecutionVertex的一次执行对应一个Execution, 在分配资源的时候会依照SlotSharingGroup和CoLocationConstraint确定
	 * 	分配时候会考虑slot重用的情况。
	 *
	 * todo
	 * 	 在所有的节点资源都获取成功后,会逐一调用Execution.deploy() 来部署Execution,
	 * 	 使用TaskDeploymentDescriptor来描述Exection, 并提交到分配该Exection的slot对应的TaskManager, 最终分配给对应的TaskExecutor执行
	 *
	 * @throws JobException
	 */
