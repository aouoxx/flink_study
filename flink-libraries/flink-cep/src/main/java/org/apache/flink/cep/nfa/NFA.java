...
public class NFA<T> {
	...
	/**
	 * 将工厂中的所有顶点,也就是状态states放到了NFA对象的一个Map中
	 * Key 为状态的Name   也就是用户代码中的 中(.next("Name"))
	 * @param validStates
	 * @return
	 */
	...
	private Collection<Map<String, List<T>>> doProcess(
		...
		// 装nfa匹配到一半没有结束的数据,也就是半匹配
		final PriorityQueue<ComputationState> newPartialMatches = new PriorityQueue<>(NFAState.COMPUTATION_STATE_COMPARATOR);
		// 装成功匹配完成的数据,用于返回,调用用户的方法去处理结果
	...
	private OutgoingEdges<T> createDecisionGraph(
		...
		// todo 这里为什么使用堆栈, 连接下面代码 [states.push(stateTransition.getTargetState());]
