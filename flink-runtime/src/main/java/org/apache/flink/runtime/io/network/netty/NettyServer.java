...
class NettyServer {
	...
	/**
	 * todo  NettyServer会启动的时候会配置水位线, 如果Netty输出缓冲中的字节数超过了高水位值, 我们会等到其降低到
	 * 		水位置以下才继续写入数据。通过水位线机制确保不向网络中写入太多数据。
	 */
	int init(
		...
		/**
		 * todo 配置水位线，确保不往网络中写入太多的数据
		 * 		当输出缓冲中的字节数超过高水位值, 则Channel.iswritable()会返回false
		 * 		当输出缓冲中的字节数低于低水位值, 则Channel.iswritable()会重新返回true
		 */
