...
public class TaskExecutor extends RpcEndpoint implements TaskExecutorGateway {
	...
	public TaskExecutor(
		...
		// todo 启动心跳  jobManagerHeartbeatManager[ 创建一个与 JM 通信的心跳管理器]  resourceManagerHeartbeatManager[创建一个与 RM 通信的心跳管理器]
	...
	/**
	 * todo 启动TaskExecutor的服务
	 * @throws Exception
	 */
	private void startTaskExecutorServices() throws Exception {
		try {
			// todo 与集群的 ResourceManager 建立连接（并创建一个 listener）
			// start by connecting to the ResourceManager
			resourceManagerLeaderRetriever.start(new ResourceManagerLeaderListener());

			// todo: taskSlotTable 启动
	...
	public CompletableFuture<Acknowledge> submitTask(
		...
		try {
			...
			// todo 由JobManager分配的用于恢复的状态
			final JobManagerTaskRestore taskRestore = tdd.getTaskRestore();

			// todo 创建TaskStateManager
			...
			try {
				// todo 将task 放入taskSlotTable#taskslotMappings中
			...
			if (taskAdded) {
				 ...
				 * 	  task开始执行
				 * 	  一个task一个独立的线程
				 *	  TM 接收到请求后,首先构建Task(Runnable) 然后通过task.startTaskThread() 将这个线程运行起来, 真正执行任务的是run 函数
				 */
	...
	public CompletableFuture<Acknowledge> triggerCheckpoint(
		...
		/**
		 * todo
		 * 	一个TaskExecutor中可能有多个Task正在运行, 因而需要根据触发checkpoint的ExecutionID找到UI应的Task
		 * 	然后调用 Task.triggerCheckpointBarrier()方法
		 */
		final Task task = taskSlotTable.getTask(executionAttemptID);
	...
	public CompletableFuture<Acknowledge> requestSlot(
		...
		try {
			// todo 是否连接到ResourceManager 判断发送请求的RM是否是当前TaskExecutor注册的
			...
			if (taskSlotTable.isSlotFree(slotId.getSlotNumber())) {
				// todo allocateSlot 分配slot  根据RM的命令分配自己的slot
			...
			// todo 将分配的slot 提供给发送请求的JobManager
			if (jobManagerTable.contains(jobId)) {
				// todo 如果和对应JobManager已经建立连接, 则向jobManager 提供slot
				offerSlotsToJobManager(jobId);
			} else {
				// todo 否则 先和jobManager建立连接, 连接建立后调用 offerSlotsToJobManager(jobId) 方法
				try {
	...
	private void connectToResourceManager() {
		...
		// todo  构建TaskExecutor Registration
	...
	private void establishResourceManagerConnection(
		...
		slotReportResponseFuture.whenCompleteAsync(
			(acknowledge, throwable) -> {
				if (throwable != null) {
					// todo 重新连接 resourcemanager 抛出异常
	...
	// ------------------------------------------------------------------------
	// todo slot被分配之后 taskExecutor 需要将对应的slot提供给jobManager
	//   而这正是通过offerSlotsToJobManager(jobID) 方法来实现的
	private void offerSlotsToJobManager(final JobID jobId) {
		...
		if (jobManagerConnection == null) {
			...
			if (taskSlotTable.hasAllocatedSlots(jobId)) {
				...
				// todo 获取JobMaster的GateWay
				final JobMasterGateway jobMasterGateway = jobManagerConnection.getJobManagerGateway();

				// todo 获取分配给jobId的所有TaskSlot, 这里只是获取状态为Allocated的slot
		...
		return (Iterable<SlotOffer> acceptedSlots, Throwable throwable) -> {
			if (throwable != null) { // todo 发生异常
				if (throwable instanceof TimeoutException) {
					...
					// todo 发生异常, 则释放所有的slot, 并将其归还给对应的RM
				...
				if (isJobManagerConnectionValid(jobId, jobMasterId)) {
					...
					// todo 被JobMaster 确认接受的slot 标记为Active状态
	...
	private void freeSlotInternal(AllocationID allocationId, Throwable cause) {
		...
		try {
			...
			// todo 尝试释放allocationId 绑定的slot
			final int slotIndex = taskSlotTable.freeSlot(allocationId, cause);

			if (slotIndex != -1) { // todo 成功释放slot

				if (isConnectedToResourceManager()) {
					// the slot was freed. Tell the RM about it
					ResourceManagerGateway resourceManagerGateway = establishedResourceManagerConnection.getResourceManagerGateway();
					// todo 告知resourcemanager 当前slot可用
				...
				if (jobId != null) {
					// todo 如果和AllocationID 绑定的Job没有分配slot了, 那么可以断开和JobMaster的连接了
					closeJobManagerConnectionIfNoAllocatedResources(jobId);
				}
			}
		} catch (SlotNotFoundException e) {
	...
	private class SlotActionsImpl implements SlotActions {
		 ...
		 * 	监控的手段是: 操作前先注册一个timeout监控, 操作完成后在取消这个监控, 如果这个期间timeout了, 就会调用这个方法
