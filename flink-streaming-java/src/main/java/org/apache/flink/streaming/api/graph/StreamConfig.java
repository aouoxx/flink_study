 ...
 *
 * todo
 * 	对于每一个StreamOperator 也就是StreamGraph中的每一个streamGraph,
 * 		在生成JobGraph的过程中StreamingJobGraphGenerator 都会创建一个对应的StreamConfig
 * 	    StreamConfig 中保存了这个算子operator在运行时需要所有配置信息, 这些信息都是通过 key/value的形式存储在Configuration中的
 */
@Internal
