...
public abstract class ClusterEntrypoint implements AutoCloseableAsync, FatalErrorHandler {
	...
	private void runCluster(Configuration configuration) throws Exception {
		synchronized (lock) {
			...
			/**
			 * todo
			 * 	 创建DefaultDispatcherResourceManagerComponentFactory 工厂的工厂
			 * 	 内部包含这个三个实例,就是三个成员变量
			 *
			 *  	 @Nonnull
			...
			// todo 创建ResourceManage,创建、启动Dispatcher，启动ResourceManage
	...
	protected void initializeServices(Configuration configuration) throws Exception {
		...
		synchronized (lock) {
			...
			/**
			 * todo 初始化一个专门来做IO的线程池
			 */
			ioExecutor = Executors.newFixedThreadPool(
				Hardware.getNumberCPUCores(),
				new ExecutorThreadFactory("cluster-io"));
			// todo haservices = ZookeeperHaServices
			haServices = createHaServices(configuration, ioExecutor);
			/**
			 * todo 初始化一个BlobServer
			 * 		主要管理一些大文件的上传,比如用户作业的jar包, TM 上传log文件等
			 * 		Blob 是指二进制大对象 也就是 英文 Binary Large Object 的缩写
			 */
			...
			/**
			 * ArchivedExecutionGraphStore 存储 execution graph 的服务, 默认有两种实现,
			 * 	1， MemoryArchivedExecutionGraphStore 主要是在内存中缓存
			 * 	2.  FileArchivedExecutionGraphStore 会持久化的文件系统, 也会在内存中缓存
			 * 	默认实现是基于File
			 */
	...
	/**
	 * 获取ha模式, 一般我们都设置为zookeeper模式
	 * 		在flink-conf.yaml 配置文件中, 我们回去配置high-availability=zookeeper
	 * @param configuration
	 * @param executor
	 * @return
	 * @throws Exception
