...
public abstract class AbstractStreamOperator<OUT>
	...
	public final void initializeState() throws Exception {
		...
		/**
		 * todo 创建StreamOperatorStateContext 这一步会进行状态的恢复
		 * 		这样operatorStateBackend 和 KeyedStateBackend就可以恢复到最后一次checkpoint的状态
		...
		try {
			// todo StateInitializationContext 对外暴露了 state backend，timer service manager 等，operator 可以借助它来进行状态初始化
			...
			// todo 进行状态初始化, 在子类中实现, 比如调用UDF的状态初始化方法
	...
	public final OperatorSnapshotFutures snapshotState(long checkpointId, long timestamp, CheckpointOptions checkpointOptions,
		...
		try {
			...
			// todo rawstate 要在子类中自己实现raw state的快照写入, timer是作为raw keyed state写入的
			snapshotInProgress.setKeyedStateRawFuture(snapshotContext.getKeyedStateStreamFuture());
			snapshotInProgress.setOperatorStateRawFuture(snapshotContext.getOperatorStateStreamFuture());

			// todo 写入 managed state 快照
			if (null != operatorStateBackend) {
				snapshotInProgress.setOperatorStateManagedFuture(
					operatorStateBackend.snapshot(checkpointId, timestamp, factory, checkpointOptions));
			}
			// todo 写入 managed keyed state 快照
