...
public class CheckpointedInputGate implements PullingAsyncDataInput<BufferOrEvent> {
	...
	public Optional<BufferOrEvent> pollNext() throws Exception {
		while (true) {
			...
			if (bufferStorage.isEmpty()) {
				// todo 如果buffer中的数据为空, 直接从inputGate中获取下一个BufferOrEvent
				next = inputGate.pollNext();
			}
			else {
				// TODO: FLINK-12536 for non credit-based flow control, getNext method is blocking
				// todo 否则从bufferStorage拿到下一个BufferOrEvent
				next = bufferStorage.pollNext();
				if (!next.isPresent()) { // todo 如果bufferStorage中的数据被处理完了, 轮训
					return pollNext();
				}
			}
			...
			// todo 如果一个channel 阻塞了, 说明还有其他channel barrier没有到来, 把阻塞的channel元素保存在bufferStorage中
			if (barrierHandler.isBlocked(offsetChannelIndex(bufferOrEvent.getChannelIndex()))) {
				// todo 如果这个channel 还是被block， 则继续把BufferOrEvent存储到Buffer中
				// if the channel is blocked, we just store the BufferOrEvent
				bufferStorage.add(bufferOrEvent);
				if (bufferStorage.isFull()) {
					// todo 判断buffer已满,可以通过taskManager配置的 task.checkpoint.alignment.max-size修改
					barrierHandler.checkpointSizeLimitExceeded(bufferStorage.getMaxBufferedBytes());
					bufferStorage.rollOver();
				}
			}
			// todo 如果这个channel不再被阻塞, 且下一条纪录是数据, 则返回此数据
			else if (bufferOrEvent.isBuffer()) {
				return next;
			}
			// todo 如果下一个是barrier, 且流没有结束, 这个channel收到了barrier
			...
			// TODO 如果下一个是带有cancel标记的barrier, 则进行processCancellationBarrier处理
			// todo 标记某次checkpoint应该被取消,取消正在对其的操作, 恢复正常的数据处理
