...
/**
 * todo Task产出的每一个ResultPartition都有一个关联的ResultPartitionWriter,
 * 		同时也都有一个独立的LocalBufferPool负责提供写入数据所需的Buffer. ResultParition实现了ResultParitionWriter
 */
public class ResultPartition implements ResultPartitionWriter, BufferPoolOwner {
	 ...
	 * 	ResultSubpartition的数量由下游消费Task和DistributionPartition了决定
	 * 	例如: 如果是Forward 则下游只有一个消费者,如果是SHUFFLE, 则下游消费者的数量和下游算子的并行度一样。
	 */
	protected final ResultSubpartition[] subpartitions;

	// todo ResultPartitionManager管理当前TaskManager所有的ResultPartition
	...
	// todo 向ResultPartition 添加一个BufferConsumer, ResultPartition会将其转交给对应的ResultSubPartition
	@Override
