...
public abstract class StreamTask<OUT, OP extends StreamOperator<OUT>>
	...
	/**
	 * todo
	 * 		triggerCheckpoint是触发checkpoint的源头, 会向下游注入CheckpointBarrier
	 * 	    下游的其他任务在收到 CheckpointBarrier后调用triggerCheckPointOnBarrier
	 */
	...
	protected static final class AsyncCheckpointRunnable implements Runnable, Closeable {
		...
		private void reportCompletedSnapshotStates(
			...
			/**
			 * todo 上报 快照状态
			 *   当任务上触发检查点或保存点时, 它将为其拥有的所有流操作符实例创建快照。然后通过此接口报告来自任务的所有操作符快照
			 *   典型的实例将把报告的状态分配并转发给相关方, 比如检查点协调器或本地状态存储。
			 */
	...
	private static final class CheckpointingOperation {
		...
		public void executeCheckpointing() throws Exception {
			...
			try {
				// todo 同步执行,依次调用每一个算子的streamOperator.snapshotState 返回结果时是一个runnableFuture
		...
		/**
		 * todo
		 * 	  每一个算子的快照被抽象为OperatorSnapshotFutures
		 */
		@SuppressWarnings("deprecation")
		private void checkpointStreamOperator(StreamOperator<?> op) throws Exception {
			if (null != op) {
				// todo 调用StreamOperator. snapshotState 方法进行快照
				//		返回的结果时runnablefuture， 可能是已经执行完了, 也可能没有执行完
