...
public class Task implements Runnable, TaskActions, PartitionProducerStateProvider, CheckpointListener, BackPressureSampleableTask {
	...
	private void doRun() {
		...
		while (true) {
			...
			// todo 首先将task状态设置为 DEPLOYING, 状态不修改成功, 不停地尝试
		...
		try {
			...
			/**
			 * todo
			 * 		加载和实例化用户Task的可执行代码 (比如SourceStreamTask)
			 * 	    invokable的triggerCheckpointBarrier 触发checkpoint
			 */
			...
			// run the invokable
			// todo 执行代码 (invokable 即为operator对象实例,比如StreamTask里) 真正开始执行这个task
			invokable.invoke();
