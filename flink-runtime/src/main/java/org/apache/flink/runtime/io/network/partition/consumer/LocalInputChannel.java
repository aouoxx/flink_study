...
public class LocalInputChannel extends InputChannel implements BufferAvailabilityListener {
		...
		synchronized (requestLock) {
			...
			if (subpartitionView == null) {
				...
				try {
					/**
					 * todo
					 * 	Local 无需网络通信, 通过ResultPartitionManager 创建一个ResultPartitionView
					 * 	LocalInputChannel 实现了BufferAvailabilityListener
					 * 		在有数据是会得到通知, notifyDataAvailable会被调用, 进而将当前channel加到InputGate的可用Channel队列中
					 */
	...
	Optional<BufferAndAvailability> getNextBuffer() throws IOException, InterruptedException {
		...
		// todo 通过ResultSubpartitionView获取
	...
	// todo 回调, 在ResultSubPartition通知ResultSubPartitionView 有数据可供消费
	@Override
	public void notifyDataAvailable() {
		// todo LocalInputChannel 通知InputGate
