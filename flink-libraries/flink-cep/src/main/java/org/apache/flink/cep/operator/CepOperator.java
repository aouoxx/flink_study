...
public class CepOperator<IN, KEY, OUT>
	...
	public void open() throws Exception {
		...
		// 创建nfa 初始化了所有的顶点statue 和 边transition 这个使用state集合已经初始化完成。
		/**
		 * NFAFactory工厂创建NFA, 这里的这个工厂是在Driver端通过用户编写的代码返回的Pattern对象转换得到的。
		 * 也就是用户env.exection的时候解析的, 工厂对象还包括了用户的所有State集合
		 */
...
//				this.nfaFactory = NFACompiler.compileFactory(pattern, timeoutHandling);
//				nfa.close();
//				nfa=nfaFactory.createNFA();
//			}
			...
			if (timestamp > lastWatermark) {
				...
				/**
				 *  窗口是把record属于的窗口边界作为定时器, 用于触发整个窗口的数据触发
				 *  但是CEP是来一个都处理所以当前水印+1作为定时器, 只有水印向前推进了就触发。
				 *
				 *  只要不是迟到的时间,它就会把当前水印时间的下一秒作为定时器 ?
				 *  然后来一条数据就触发计算 ?
				 *  cep ?
				 *
				 */
				...
				/**
				 *  把元素放在statemap中, 时间戳作为他的key，原因是后面会把所有的数据的key即 事件时间取出来,
				 *  时间放到一个优先队列, 因为事件时间会把数据先buffer起来到这个map里面
				 */
	 ...
	 * buffered elements.
	 * 这里主要是因为窗口是一批一批触发而CEP需要逐个触发，所以用（当前水印+1）当做定时器，
	 * 也就是说只要水印往前推进了就触发推进这段时间的所有计算
	 */
