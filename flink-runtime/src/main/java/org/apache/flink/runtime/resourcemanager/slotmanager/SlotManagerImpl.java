...
public class SlotManagerImpl implements SlotManager {
	...
	/**
	 * todo
	 * 	当前RM资源不足的时候,会通过ResourceActions # allocateResource(ResourceProfile) 向yarn (Yarn cluster模式)申请新资源
	 * 	会可能尝试启动新的TaskManager，也可能什么都不做
	 ...
	 * todo
	 * 	SlotManager组件在启动的时候会启动两个超时检测任务
	 * 	 1) 一个是对TaskManager 长时间处于空闲状态的检测
	 * 	 2) 一个是对Slot request超时的检测
	 *  一旦TaskExecutor长时间处于空闲状态,则会通过ResourceAction#releaseResource()回调函数释放资源
	 *  如果一个slot request超时,则会取消PendingSlotRequest, 并通过ResourcActions#notifyAllocationFailure告知resourceManager
	 *
	...
	public void start(ResourceManagerId newResourceManagerId, Executor newMainThreadExecutor, ResourceActions newResourceActions) {
		...
		// todo 检查TaskExecutor是否长时间处于idle状态
		taskManagerTimeoutCheck = scheduledExecutor.scheduleWithFixedDelay(
			() -> mainThreadExecutor.execute(
				() -> checkTaskManagerTimeouts()), // todo 超时检查
			0L,
			taskManagerTimeout.toMilliseconds(),
			TimeUnit.MILLISECONDS);
		// todo 检查slot request是否超时
		slotRequestTimeoutCheck = scheduledExecutor.scheduleWithFixedDelay(
			() -> mainThreadExecutor.execute(
				() -> checkSlotRequestTimeouts()), // todo 超时检查
			0L,
	...
	public boolean registerSlotRequest(SlotRequest slotRequest) throws ResourceManagerException {
		...
		if (checkDuplicateRequest(slotRequest.getAllocationId())) {
			...
			// todo 封装请求, 将请求封装为PendingSlotRequest
			PendingSlotRequest pendingSlotRequest = new PendingSlotRequest(slotRequest);
			pendingSlotRequests.put(slotRequest.getAllocationId(), pendingSlotRequest);

			try {
				// todo 执行请求申请分配slot的具体逻辑
	...
	/**
	 * todo 在slot Manager 中注册一个新的Task Manager
	...
	public void registerTaskManager(final TaskExecutorConnection taskExecutorConnection, SlotReport initialSlotReport) {
		...
		// todo 我们通过任务管理器的实例ID来识别它们
		// we identify task managers by their instance id
		if (taskManagerRegistrations.containsKey(taskExecutorConnection.getInstanceID())) {
			// todo 之前已经连接过, 直接报告 slot的状态
			reportSlotStatus(taskExecutorConnection.getInstanceID(), initialSlotReport);
		} else {
			// todo 第一次注册TaskManager
			...
			// todo 注册纪录 ---> 实例ID 与 对应的TaskManagerRegistration(连接,slot总数)信息
	...
	/**
	 * todo 在slotManager 中为给定的TaskManager注册slot, slot由给定的slot id标识
	...
	private void registerSlot(
		...
		// todo 创建和注册,新的这些slot  (创建一个TaskManagerSlot对象, 并加入到slots中)
		...
		if (allocationId == null) {
			// todo 查找挂起的slot [ 这个slot还没有被分配,则找到和当前slot计算资源相匹配的PendingTaskManagerSlot]
		...
		// todo 如果队列中挂起的slot为null, 直接更新
		if (pendingTaskManagerSlot == null) {
			// todo 更新slot状态 [有两种可能, 1) slot已经被分配   2) 没有匹配的PendingTaskManagerSlot ]
			updateSlot(slotId, allocationId, jobId);
		} else {
			// todo 新注册的slot能够满足pendingTaskManagerSlot的要求, 尝试将该slot分配给当前的Slot Request
			pendingSlots.remove(pendingTaskManagerSlot.getTaskManagerSlotId());
			final PendingSlotRequest assignedPendingSlotRequest = pendingTaskManagerSlot.getAssignedPendingSlotRequest();

			// todo pendingTaskManagerSlot 可能有关联的PendingSlotRequest
			if (assignedPendingSlotRequest == null) {
				// todo 没有关联的PendingSlotRequest,则尝试再次从pendingSlot中寻找合适的Requestslot进行分配,否则标记释放该slot为Free状态
				handleFreeSlot(slot);
			} else {
				// todo 表示slot 要被分配给某个请求
				assignedPendingSlotRequest.unassignPendingTaskManagerSlot();
				// todo 有关联的PendingSlotRequest, 则这个request可以满足, 分配slot
	...
	/**
	 * todo 创建一个TaskManagerSlot对象，并加入slots中
	 * @param slotId
	 * @param resourceProfile
	 * @param taskManagerConnection
	 * @return
	 */
	@Nonnull
	...
	private void internalRequestSlot(PendingSlotRequest pendingSlotRequest) throws ResourceManagerException {
		...
		// todo 首先从Free状态的已注册的slot中选择符合要求的slot (cpu, heap-direct-native-network-mem 资源> 请求需求的资源)
		OptionalConsumer.of(findMatchingSlot(resourceProfile))
			// todo 找到了符合条件的slot, 将该slot尝试分配给该pendingSlotRequest
			.ifPresent(taskManagerSlot -> allocateSlot(taskManagerSlot, pendingSlotRequest))
			/**
			 * todo 参考具体的方法 [fulfillPendingSlotRequestWithPendingTaskManagerSlot]
			 */
	...
	private void fulfillPendingSlotRequestWithPendingTaskManagerSlot(PendingSlotRequest pendingSlotRequest) throws ResourceManagerException {
		...
		if (!pendingTaskManagerSlotOptional.isPresent()) {
			// todo 向RM( 如Yarn) 再次申请container; 并用java cmd 命令启动对应的Flink TaskManager
			pendingTaskManagerSlotOptional = allocateResource(resourceProfile);
		}

		OptionalConsumer.of(pendingTaskManagerSlotOptional)
			// todo 将PendingTaskManagerSlot指派给对应的PendingSlotRequest
	...
	/**
	 * todo
	 * 	在SlotManager中具体的slot申请分配的逻辑方法为: allocateSlot(taskManagerSlot, pendingSlotRequest)
	 * 	其主要通过TaskExecutorGateWay的RPC接口代理 调用gateway.requestSlot() 方法向对应的TaskExecutor申请请求分配slot资源
	 * 	该TaskExecutorGateWay的RPC方法 gateWay.requestSlot() 调用源码如下
	 *
	...
	private void allocateSlot(TaskManagerSlot taskManagerSlot, PendingSlotRequest pendingSlotRequest) {
		// todo 检测 taskManager的slot状态是否空闲
		Preconditions.checkState(taskManagerSlot.getState() == TaskManagerSlot.State.FREE);

		// todo 获取taskManager 的连接信息
		TaskExecutorConnection taskExecutorConnection = taskManagerSlot.getTaskManagerConnection();
		// todo 获取TaskManager的Gateway ...
		TaskExecutorGateway gateway = taskExecutorConnection.getTaskExecutorGateway();

		// todo  缓存task slot的回调completableFuture
		...
		// todo task manager的slot处理: 将slot的状态设置为PENDING
		taskManagerSlot.assignPendingSlotRequest(pendingSlotRequest);

		pendingSlotRequest.setRequestFuture(completableFuture);

		// todo 如果有PendingTaskManager 指派给当前pendingSlotRequest, 要先解除关联
		returnPendingTaskManagerSlotIfAssigned(pendingSlotRequest);
		// todo 获取实例的TaskManagerRegistration
		TaskManagerRegistration taskManagerRegistration = taskManagerRegistrations.get(instanceID);
		...
		// todo taskManagerRegistration 标注为已经使用
		taskManagerRegistration.markUsed();

		// RPC call to the task manager
		// todo 请求slot, 通过RPC调用向TaskExecutor 申请 请求slot资源
		// todo 分配完成之后,通知tm 提供slot给jm
		...
		// todo PendingSlotRequest请求完成的回调函数(PendingSlotRequest)请求完成可能是由于上面RPC调用完成, 也可能是因为PendingSlotRequest被取消
		completableFuture.whenCompleteAsync(
			(Acknowledge acknowledge, Throwable throwable) -> {
				try {
					if (acknowledge != null) { //todo 如果请求成功,则取消pendingSlotRequest, 并更新slot状态 PENDING-> ALLOCATED
						updateSlot(slotId, allocationId, pendingSlotRequest.getJobId());
					} else {
						if (throwable instanceof SlotOccupiedException) { //todo 这个slot已经被占用了, 更新状态
							SlotOccupiedException exception = (SlotOccupiedException) throwable;
							updateSlot(slotId, exception.getAllocationId(), exception.getJobId());
						} else {
							// todo 请求失败,将pendingSlotRequest从TaskManagerSlot中移除
							removeSlotRequestFromSlot(slotId, allocationId);
						}

						if (!(throwable instanceof CancellationException)) {
							// todo slot request请求失败, 会进行重试
		...
		if (!taskManagerRegistrations.isEmpty()) {
			...
			for (TaskManagerRegistration taskManagerRegistration : timedOutTaskManagers) {
				if (waitResultConsumedBeforeRelease) {
					...
					// todo 通过ResourceAction#releaseResource() 回调函数, 超时释放资源
		...
		if (!pendingSlotRequests.isEmpty()) {
			...
			while (slotRequestIterator.hasNext()) {
				...
				if (currentTime - slotRequest.getCreationTimestamp() >= slotRequestTimeout.toMilliseconds()) {
					...
					resourceActions.notifyAllocationFailure( // todo 告知ResourceManager
