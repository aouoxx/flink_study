...
public class CopyOnWriteStateMap<K, N, S> extends StateMap<K, N, S> {
	...
	public S get(K key, N namespace) {
		...
		for (StateMapEntry<K, N, S> e = tab[index]; e != null; e = e.next) {
			...
			if ((e.hash == hash && key.equals(eKey) && namespace.equals(eNamespace))) {
				// todo 一旦get当前数据,为了防止应用层修改数据内部的属性值,所以必须保证这是一个最新的Entry, 并更新器stateVersion
				// copy-on-write check for state
				/**
				 * todo
				 * 	首先检查当前state,是否为旧版本数据
				 * 	如果value是旧版本,就必须深拷贝一个value
				 * 	否则value是新版本,直接返回给应用成
				 */
				if (e.stateVersion < requiredVersion) {
					 ...
					 * todo
					 * 	此时还有两种情况:
					 * 		1) 如果当前Entry是旧版本,则Entry也需要拷贝一份,按照handleChainedEntryCopyOnWrite策略拷贝即可
					 * 	    2) 如果当前Entry是新版本数据,则不需要拷贝,直接修改其state即可
					 */
	...
	private StateMapEntry<K, N, S> putEntry(K key, N namespace) {
		...
		for (StateMapEntry<K, N, S> e = tab[index]; e != null; e = e.next) {
			// todo 如果根据key和namespace找到了对应的Entry, 则认为是修改数据
			// 普通的HashMap结果有一个Key, 这里Key和namespace的组合当做key
			if (e.hash == hash && key.equals(e.key) && namespace.equals(e.namespace)) {
				/**
				 * todo
				 * 	 entryVersion表示entry创建时的版本号
				 * 	 highestRequiredSnapshotVersion表示正在进行中的那些snapshot的最大版本号
				 * 	 entryVersion小于highestRequiredSnapshotVersion,说明Entry的版本小于当前某些Snapshot的版本号
				 * 	 即: 当前Entry是旧版本的数据, 当前Entry被其他snapshot持有。
				 * 	 为了保证snapshot的数据正确性,这里必须为e创建新的副本,且e之前的某些元素也需要copy副本
				 * 	 handleChainedEntryCopyOnWrite方法将会进行相应的copy操作,并返回e的新副本
				 * 	 然后handleChainedEntryCopyOnWrite方法返回e的副本给上层,进行数据的修改操作。
				 */
				// copy-on-write check for entry
				 ...
				 * 	 说明当前Entry创建时的版本比所有snapshot的版本高。
				 * 	 即当前Entry是新版本的数据
				 */
				return e;
			}
	 ...
	 * todo
	 * 	releaseSnapshot 方法将相应的 snapshotVersion 从 snapshotVersions 中 remove，
	 * 	并将 snapshotVersions 的最大值更新到 highestRequiredSnapshotVersion，
	...
	void releaseSnapshot(int snapshotVersion) {
		...
		synchronized (snapshotVersions) {
			// todo 将相应的snapshotVersion从snapshotVersions中remove
			Preconditions.checkState(snapshotVersions.remove(snapshotVersion), "Attempt to release unknown snapshot version");
			// 将snapshotVersions的最大值更新到highestRequiredSnapshotVersion,
			// 如果snapshotVersions为空, 则highestReqiredSnapshotVersion更新为0
	...
	StateMapEntry<K, N, S>[] snapshotMapArrays() {
		...
		synchronized (snapshotVersions) {
			// stateMapVersion 版本 +1,赋值给 highestRequiredSnapshotVersion, 并加入到 snapshotVersions
		...
		/**
		 * 将现在 primary 和 incrementalRehashTable的元素浅拷贝一份到copy中
		 * copy策略: copy数组长度为 primary中剩余的桶数 + Increment 中有数据的桶数。
		 * 		primary中,   剩余的数据放在copy数组的迁移
		 * 	    increment中, 低位数据随后
		 * 	   	increment中, 高位数据放到copy数组的最后
		 */
	 ...
	 *
	 * todo
	 *  每次调用get,put,containKey,remove操作时, 都会调用computeHashForOperationAndDoIncrementalRehash方法
	 *  触发迁移操作。
	 *  computeHashForOperationAndDoIncrementalRehash方法作用:
	 *     1) 检测是否处于rehash中, 如果正在rehash就会调用incrementalRehash迁移一波数据
	 *     2) 计算key和namespace对应的hashCode
	 */
	@SuppressWarnings("unchecked")
	private void incrementalRehash() {
		...
		// 每次至少迁移 MIN_TRANSFERRED_PER_INCREMENTAL_REHASH(默认4)个元素到新桶
		while (transferred < MIN_TRANSFERRED_PER_INCREMENTAL_REHASH) {

			StateMapEntry<K, N, S> e = oldMap[rhIdx];

			/**
			 * 每次e都执行 e.next, e不为空, 表示当前桶中还有元素未遍历,需要继续遍历
			 * 每次迁移都必须保证,整个桶被迁移完,不能是某个桶的迁移到一半
			 */
			while (e != null) {
				// copy-on-write check for entry
				// 遇到版本比 highestRequiredSnapshotVersion小的元素, 则copy一份
				if (e.entryVersion < requiredVersion) {
					e = new StateMapEntry<>(e, stateMapVersion);
				}
				// 保存下一个要迁移的节点到n
				StateMapEntry<K, N, S> n = e.next;
				// 迁移当前元素e到新的table中, 插入到链表头部
			...
			oldMap[rhIdx] = null;
			// rhIdx== oldCapacity 之前的桶全部迁移完了, 进行舒适化操作。
	 ...
	 * todo 为Entry创建新的副本, 且链表中Entry e之前某些元素也需要copy 副本, 最后返回e 的副本
	 *
	 * 从头结点到要修改的 Entry 节点依次遍历桶中元素，都是使用 current.entryVersion < highestRequiredSnapshotVersion
	 *   来判断当前节点的创建创建时的版本是否低于 highestRequiredSnapshotVersion。
	 *
	 * 		如果低于则 current 节点被 Snapshot 引用，所以需要 new 一个新的 Entry，也就是所谓的拷贝一个副本。
	 * 		否则不用拷贝。
	 *
	 * 在新创建 Entry 时，新 Entry 的 entryVersion 要更新为当前 StateMap 的 version，
	 * 	表示这是一个新版本的 Entry，并没有被 Snapshot 引用。
	 * 这样之后再要修改该 Entry 时直接修改该 Entry 即可，不需要再拷贝一份副本了。
	 */
	private StateMapEntry<K, N, S> handleChainedEntryCopyOnWrite(
		...
		// todo current 指向当前桶的头节点
		StateMapEntry<K, N, S> current = tab[mapIdx];
		StateMapEntry<K, N, S> copy;

		/**
		 * todo
		 * 	判断头节点创建时的版本是否低于 highestRequiredSnapshotVersion
		 * 	如果低于,则current节点被snapshot引用,所以要new 一个新的Entry
		 */
		...
		// todo 依次遍历当前桶的元素, 直到遍历到untilEntry节点, 也就是我们要修改的Entry节点
		while (current != untilEntry) {

			//advance current
			current = current.next;
			// todo current版本小于highestRequiredSnapshotVersion, 则需要拷贝,否则不用copy
			if (current.entryVersion < required) {
				// copy and advance the current's copy
				// todo entryVersion表示创建Entry时的version, 所以新创建的Entry对应entryVersion也要更新为当前StateMap的version
