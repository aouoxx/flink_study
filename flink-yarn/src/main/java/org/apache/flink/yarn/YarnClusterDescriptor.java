...
public class YarnClusterDescriptor implements ClusterDescriptor<ApplicationId> {
	...
	private ClusterClientProvider<ApplicationId> deployInternal(
		...
		/**
		 * yarn.scheduler.minimum-allocation-mb yarn单个容器最小分配的大小
		 */
		...
		final ClusterEntrypoint.ExecutionMode executionMode = detached ?
				ClusterEntrypoint.ExecutionMode.DETACHED  // 分离模式 客户端提交后可以退出
				: ClusterEntrypoint.ExecutionMode.NORMAL;  // 客户端一直保持连接
	...
	private ApplicationReport startAppMaster(
		...
		if (HighAvailabilityMode.isHighAvailabilityModeActivated(configuration)) {
			...
			appContext.setMaxAppAttempts(
					configuration.getInteger(
							// TODO yarn.application-attempts
							YarnConfigOptions.APPLICATION_ATTEMPTS.key(),
							//  TODO DEFAULT_RM_AM_MAX_ATTEMPTS = 2
		...
		// only for per job mode, todo 只用于per-JOB模式
		if (jobGraph != null) {
		...
		//todo 将执行封装的map(AM的环境信息,类路径) 设置到容器里
