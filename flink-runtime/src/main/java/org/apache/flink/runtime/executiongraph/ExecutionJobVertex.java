...
/**
 *  todo
 *  	在ExecutionGraph中,节点对应的类是ExecutionJobVertex, 与之对应的就是JobGraph中的JobVertex
 *  	每一个ExecutionJobVertex 都是由一个JobVertex生成的
 */
public class ExecutionJobVertex implements AccessExecutionJobVertex, Archiveable<ArchivedExecutionJobVertex> {
	...
	/**
	 * todo
	 *   ExectionVertex 对应一个并行的子任务
	 *   Flink Job可以指定任务的并行度, 在实际运行时, 会有一个并行的任务同时在执行,对应的就是ExectionVertex
	 *   ExectionVertex 是并行任务的一个子任务, 算子的并行度是多少, 那么就会有多少个ExectionVertex
	 */
	...
	public ExecutionJobVertex(
		...
		// todo ExecutionJobVertex 中的 ExecutionVertex个个数和并行度一致
		...
		// TODO  ExectionJobVertex的输出[IntermediateResult]的个数和 JobVertex输出 IntermediaDataSet的个数一样
		this.producedDataSets = new IntermediateResult[jobVertex.getNumberOfProducedIntermediateDataSets()];

		for (int i = 0; i < jobVertex.getProducedDataSets().size(); i++) {
			final IntermediateDataSet result = jobVertex.getProducedDataSets().get(i);
			/**
			 * todo 一个IntermediateResult对应parallelism个IntermediateResultPartition
			 */
	...
	public void connectToPredecessors(Map<IntermediateDataSetID, IntermediateResult> intermediateDataSets) throws JobException {
		...
		for (int num = 0; num < inputs.size(); num++) {
			...
			// todo 通过ID获取当前jobEdge的输入[IntermediateData]所对应的 IntermediateResult
			IntermediateResult ires = intermediateDataSets.get(edge.getSourceId());
			if (ires == null) {
				throw new JobException("Cannot connect this job graph to the previous graph. No previous intermediate result found for ID "
						+ edge.getSourceId());
			}
			// todo 将IntermediateResult 加入到当前ExecutionJobVertex的输入中
			this.inputs.add(ires);
			// todo 为IntermediateResult 注册consumer, 就是当前节点
			int consumerIndex = ires.registerConsumer();
			// todo 由于每一个并行度都对应一个节点。 所以要把每个节点都和前面中间结果相连
