...
public abstract class ResourceManager<WorkerType extends ResourceIDRetrievable>
	...
	private void startResourceManagerServices() throws Exception {
		try {
			...
			// todo 初始化操作, 并启动yarn的resource 和 node的客户端
			initialize();
	...
	public CompletableFuture<Acknowledge> sendSlotReport(ResourceID taskManagerResourceId, InstanceID taskManagerRegistrationId, SlotReport slotReport, Time timeout) {
		...
		if (workerTypeWorkerRegistration.getInstanceID().equals(taskManagerRegistrationId)) {
			// todo slotManager注册该Taskmanager的  信息
	...
	public CompletableFuture<Acknowledge> requestSlot(
		...
		// todo 获取对应任务的jobId的jobMaster rpc-gateway接口代理
		...
		if (null != jobManagerRegistration) {
			if (Objects.equals(jobMasterId, jobManagerRegistration.getJobMasterId())) {
				...
				try {
					/**
					 * todo ResourceManager 内部的slotManager 去向Yarn的ResourceManger请求资源
					 * 		委托内部的slotManager的registerSlotRequest方法进行slot资源的请求申请
					 */
	...
	protected void releaseResource(InstanceID instanceId, Exception cause) {
		...
		if (worker != null) {
			// todo 释放停止对应的worker, 并关闭到对应TaskManager的链接
			// todo stopWorker(worker) 具体实现和不同的ResourceManager的实现有关
	...
	private class ResourceActionsImpl implements ResourceActions {
		...
		public void releaseResource(InstanceID instanceId, Exception cause) {
			...
			// todo 调用具体ResourceManager的relaseResouce方法
		 ...
		 * todo
		 * 	 分配资源
		 * 	 申请新的资源,具体行为和不同的ResourceManager的实现有关
		 * 	 其返回的列表相当于是承诺即将分配的资源
		 * 	 	[在yarn模式下,就是RequestYarnContainer, 申请container并启动对应的TaskManager]
