...
/**
 * todo
 * 	 Catalog 接口能够支持数据库, 表, 函数, 甚至于分区等多种抽象。
 * 	 FlinkSQL 提供了两个Catalog的具体实现, 即GenericInMemoryCatalog 和 HiveCatalog。
 *
 * 	 GenericInMemoryCatalog>> 将所有元数据存储在内存中
 * 	 HiveCatalog >> 通过HiveShim 连接hive Metastore的实例,提供元数据持久化的能力。
 * 	 通过HiveCatalog, 可以访问到Hive中管理的所有表, 从而在Batch模式下使用。
 * 	 通过HiveCatalog, 也可以使用Hive中定义的UDF, FlinkSQL 提供了对于Hive UDF的支持。
 *
 */
@PublicEvolving
